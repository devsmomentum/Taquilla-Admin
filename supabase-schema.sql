...
GROUP BY u.id, u.name, u.email, u.is_active, u.created_at, u.created_by;

-- Vista: estadísticas de loterías
...
SELECT 
  ...
  COUNT(DISTINCT CASE WHEN b.is_winner THEN b.id END) AS total_winners,
  COALESCE(SUM(b.amount), 0) AS total_bet_amount,
  COALESCE(SUM(CASE WHEN b.is_winner THEN b.potential_win ELSE 0 END), 0) AS total_payout,
